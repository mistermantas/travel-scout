...
interface AccorSnapshotFile {
  fetched_at: string;
  source_tool: string;
  pricing_note: string;
  listings: AccorSnapshotListing[];
}

interface AccorSnapshotListing {
...
export class AccorSnapshotSource implements SourceAdapter {
  readonly name = "accor_snapshot";

  constructor(private readonly path: string) {}

  async search(cities: CityConfig[], windows: DateWindow[]): Promise<Listing[]> {
    const data = JSON.parse(await readFile(this.path, "utf8")) as AccorSnapshotFile;
  ...
  private normalize(item: AccorSnapshotListing, data: AccorSnapshotFile): Listing {
